async function choropleth() {
    let svg = d3.select("#children_map").select("svg");
    let width = "1200";
    let height = "600";
    ...
    let stateData = await d3.csv("../scripts/children_female.csv");
...
console.log(d3.min(stateData,function (d) {
    ...
    d3.select("#children_map").select("svg").select("#mapLayer").selectAll("path")
        .data(json.features)
        .join("path")
